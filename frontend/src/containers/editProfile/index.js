...
import {createNumber} from '../../service/contactService';
...
class Index extends Component {
  ...
  componentDidMount() {
    if(this.props.contact && this.props.contact.selectedContact) {
      ...
      const newContact = ContactService.createContactProfile('Full name', '', false, 'Email', [createNumber('Cell', 'Number')]);
  ...
  onContactDataSave = () => {
    if(this.state.dirtyContact){
      ...
      if(validationResponse.valid){
        this.props.actions.contact.saveContact(this.state.dirtyContact);
